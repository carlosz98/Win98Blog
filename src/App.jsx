...
function App() {
  ...
  const [solitaireShow, setSolitaireShow] = useState(false);
  ...
  const [rightClickBin, setRightClickBin] = useState(false)
  const [iconBeingRightClicked, setIconBeingRightClicked] = useState({});
  const [rightClickIcon, setRightClickIcon] = useState(false);
  const [rightClickDefault, setRightClickDefault] = useState(false);
  ...
  const UserCreatedFolderRef = useRef([]);
    ...
    const allPicture = desktopIcon.filter(picture => picture.type === '.jpeg');
...
useEffect(() => {
    ...
    const connectWebSocket = async () => {
      try {
        ...
        socket.current.onmessage = (event) => {
          ...
          if (data.count !== undefined) { setOnlineUser(data.count); }
          if (data.key) { setKeyChatSession(data.key); } 
          if (data.ring) { setRingMsn(true) }
  ...
  useEffect(() => {
    if(allowNoti){
      if (chatData.length) { endOfMessagesRef.current?.scrollIntoView({ behavior: "smooth" }); }
      if(!MSNExpand.show || MSNExpand.hide) {
        setNotiOn(false);
        setTimeout(() => {
            clearTimeout(clearNotiTimeOut)
            setNotiOn(true);
            setNewMessage({ type: 'msn'});
        }, 100);
      }
    }
  },[chatData])
...
const handleOnDrag = (name, ref, type) => () => {
  ...
  if (iconRef && ResumeFolderRef.current && ProjectFolderRef.current) {
    ...
    const offset = 55;
    if(name === 'MyComputer' || name === 'RecycleBin') return;
    ...
    if (iconRect.left < UtilityRect.right - offset && iconRect.right > UtilityRect.left + offset && iconRect.top < UtilityRect.bottom - offset && iconRect.bottom > UtilityRect.top + offset) {
      if(name === 'Utility') return;
      setDropTargetFolder('Utility');
    }
    else if (iconRect.left < BinRect.right - offset && iconRect.right > BinRect.left + offset && iconRect.top < BinRect.bottom - offset && iconRect.bottom > BinRect.top + offset) {
      if(name === 'RecycleBin') return;
      setDropTargetFolder('RecycleBin');
    }
    else if (iconRect.left < PictureRect.right - offset && iconRect.right > PictureRect.left + offset && iconRect.top < PictureRect.bottom - offset && iconRect.bottom > PictureRect.top + offset) {
      if(name === 'Picture') return;
      setDropTargetFolder('Picture');
    }
    else if (iconRect.left < resumeFolderRect.right - offset && iconRect.right > resumeFolderRect.left + offset && iconRect.top < resumeFolderRect.bottom - offset && iconRect.bottom > resumeFolderRect.top + offset) {
      if(name === 'Resume') return;
      setDropTargetFolder('Resume');
    }
    else if (iconRect.left < projectFolderRect.right - offset && iconRect.right > projectFolderRect.left + offset && iconRect.top < projectFolderRect.bottom - offset && iconRect.bottom > projectFolderRect.top + offset) {
      if(name === 'Project') return;
      setDropTargetFolder('Project');
    }
    else if (iconRect.left < diskRect.right - offset && iconRect.right > diskRect.left + offset && iconRect.top < diskRect.bottom - offset && iconRect.bottom > diskRect.top + offset) { 
      ...
      if (validFolders.includes(currentFolder)) { setDropTargetFolder(currentFolder); }
    }
    else if (iconRect.left < desktopRect.right && iconRect.right > desktopRect.left && iconRect.top < desktopRect.bottom && iconRect.bottom > desktopRect.top) {
      setDropTargetFolder('Desktop');
    }
    else { setDropTargetFolder('Desktop'); }
  }
};

function handleShowInfolder(name, type) {
  setRightClickDefault(false);
  if (name === 'Hard Disk (C:)') { setCurrentFolder('DiskC'); setSelectedFolder({label: 'Hard Disk (C:)', img: imageMapping(name)}); setUndo(prev => [...prev, 'DiskC']); return; }
  if (name === 'Hard Disk (D:)') { setCurrentFolder('DiskD'); setSelectedFolder({label: 'Hard Disk (D:)', img: imageMapping(name)}); setUndo(prev => [...prev, 'DiskD']); return; }
  if (name === 'Resume') { setCurrentFolder('Resume'); setSelectedFolder({label: 'Resume', img: imageMapping(name)}); setUndo(prev => [...prev, 'Resume']); return; }
...
function handleShowInfolderMobile(name, type) {
  ...
  if (now - lastTapTime < 300) {
    if (name === 'Hard Disk (C:)') { setTimeout(() => { setCurrentFolder('DiskC') }, 100); setSelectedFolder({label: 'Hard Disk (C:)', img: imageMapping(name)}); setUndo(prev => [...prev, 'DiskC']); return; }
    if (name === 'Hard Disk (D:)') { setTimeout(() => { setCurrentFolder('DiskD') }, 100); setSelectedFolder({label: 'Hard Disk (D:)', img: imageMapping(name)}); setUndo(prev => [...prev, 'DiskD']); return; }
    if (name === 'Resume') { setTimeout(() => { setCurrentFolder('Resume') }, 100); setSelectedFolder({label: 'Resume', img: imageMapping(name)}); setUndo(prev => [...prev, 'Resume']); return; }
  ...
  if(loading && !login) {
    ...
    return(
      <div style={{ width: '100%', height: '100svh', background: localThemeBg }}>
        <img src={loadingSpin} alt="loading" 
          style={{ width: '30px', position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%, -50%)' }}
        />
      </div>
    )
  }
  ...
  function sortDesktopIcons(iconArr) {
    ...
    const sortedIcons = [...iconArr].sort((a, b) => {
        if (a.y === b.y) {
            if (a.x > b.x) {
                if (!lesserXItems.includes(a)) { lesserXItems.push(a); }
            }
        }
        return a.y - b.y;
    });
    const firstArr = sortedIcons.filter(item => !lesserXItems.includes(item));
    return [...firstArr, ...lesserXItems];
  }
  ...
  function handleDrop(e, name, target, oldFolderID) {
    ...
    if(droppedIcon.folderId === target) { setKey(prev => prev + 1); return; }
    if (target === 'RecycleBin') {
      setBinRestoreArr(prevArr => {
        const updatedArr = [...prevArr, { name: name, OldFolder: oldFolderID }];
    ...
    function remountRunPosition() {
      if(!RunExpand.show && !ErrorPopup) { setReMountRun(prev => prev + 1) }
    }

    function handleMouseSeen() { setDetectMouse(true) }

    function ringMsnOff() {
      if(socket.current) { socket.current.send(JSON.stringify({ ring: true })); }
      else { console.error('WebSocket is not initialized.'); }
    }

    async function createChat() {
      const filter = new Filter();
      setTimeout(() => { setSendDisable(false); }, 20000);
      setSendDisable(true);
      if (chatValue.trim().length === 0) { setSendDisable(false); return; }
      const offendedWords = badword();
      offendedWords.forEach(word => filter.addWords(word));
      const newChatVal = filter.clean(chatValue);
      const payload = { chat: newChatVal, key: KeyChatSession, mouse: detectMouse, touch: isTouchDevice, chatBotActive: chatBotActive };
      if (userNameValue.trim().length < 1) { payload.name = 'Anonymous' }
      if (userNameValue.trim().length > 0) { const cleanedName = filter.clean(userNameValue); payload.name = cleanedName; }
      if (socket.current) { socket.current.send(JSON.stringify(payload)); }
      else { console.error('WebSocket is not initialized.'); }
      setChatValue('');
  ...
  function iconFocusIcon(name) {
    ...
    const updateddesktopIcon = desktopIcon.map(icon => {
      ...
      if ('focus' in icon) { return { ...icon, focus: iconName === passedName }; }
      return { ...icon, focus: false };
    });
      ...
      if(itemName === lowerCaseName) {
        setTimeout(() => {
          if(item.type === 'userCreatedFolder') {
            item.setter({ show: true, focusItem: true, hide: false, zIndex: maxZindexRef.current + 1 });
          } else {
        ...
        if(lowerCaseName === '3dobject') { handleDoubleClickiframe('3dObject', setOpenProjectExpand, setProjectUrl, setBackTrackIe, setForwardTrackIe); handleShow('Internet'); }
        if(lowerCaseName === 'fortune') { handleDoubleClickiframe('Fortune', setOpenProjectExpand, setProjectUrl, setBackTrackIe, setForwardTrackIe); handleShow('Internet'); }
  ...
  function handleShowMobile(name) {
    ...
    if (now - lastTapTime < 300) {
      ...
      if (pictureMatch) { handleDoubleClickPhotoOpen(name, setCurrentPhoto); handleShow('Photo'); return; }
  ...
  function clippySendemailfunction() { handleClippyFunction(setClippySendemail, ClearTOclippySendemailfunction, allSetters); }
  function clippySongFunction() { handleClippyFunction(setClippySong, ClearTOSongfunction, allSetters); }
  function clippyUsernameFunction() { handleClippyFunction(setClippyUsername, ClearTOclippyUsernameFunction, allSetters); }
  ...
  function handleSetFocusItemTrue(name) {
    ...
    setState.forEach((item) => {
      ...
      if (itemName === LowerCaseName) {
        if (item.type === 'userCreatedFolder') { item.setter({ ...item, focusItem: true, zIndex: newZIndex }); }
        else { item.setter(prev => ({ ...prev, focusItem: true, zIndex: newZIndex })); }
        maxZindexRef.current = newZIndex;
      } else {
        if (item.type === 'userCreatedFolder') { item.setter({ ...item, focusItem: false }); }
        else { item.setter(prev => ({ ...prev, focusItem: false })); }
      }
    });
    setDesktopIcon(prevIcons => prevIcons.map(icon => ({ ...icon, focus: false })));
  ...
  function inlineStyleExpand(name) {
    ...
    const setState = ObjectState();
    const item = setState.find(item => item.name.split(' ').join('').toLowerCase() === passedName);
    if (item) {
      return {
        display: item.usestate.show ? 'block' : '',
        maxWidth: 'none', width: '100%', height: 'calc(100% - 37px)',
  ...
  function deleteTap(name) {
    ...
    setState.forEach(item => {
      ...
      if (itemName === passedName) {
        item.setter(prev => ({ ...prev, show: false, expand: false, hide: false }));
        if(item.type === 'userCreatedFolder') {
          item.setter({ show: false, focusItem: false, hide: true, zIndex: maxZindexRef.current + 1 });
        }
        setTap(prevTap => prevTap.filter(tapItem => tapItem.toLowerCase().split(' ').join('') !== passedName));
      }
    });
  }
}
